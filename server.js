...
const sourceURL = "https://classic.settrade.com/C04_01_stock_quote_p1.jsp?txtSymbol=";
const { JSDOM } = jsdom;
...
function getStockPrice(stock) {
    try {
        ...
        if (price == "-") {
            ...
            return { p: Number(price), c: Number(change), cp: Number(percentChange.substr(0, percentChange.length - 1)) };
        }
    } catch (ex) {
        console.log(ex);
        return { p: -1, c: -1, cp: -1 };
